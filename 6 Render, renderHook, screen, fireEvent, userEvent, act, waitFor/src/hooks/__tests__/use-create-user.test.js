...
describe('useCreateUser', () => {
  ...
  it('should set the success message', () => {
    ...
    // act только когда изменяется state
    act(() => {
      result.current.onSuccess({ name: 'John', password: 'Qwerty123!' });
    })

    expect(result.current.successMessage).toBe('User John created with password Qwerty123!');
  });
  ...
  it('should NOT throw an error', async () => {
    ...
    await expect(result.current.onSubmit({ password: 'Qwerty123!' })).resolves.toBe();
  });
})